...
BRANCH = '{{ cookiecutter.default_branch }}'.strip() or 'main'
...
def main():
    ...
    try:
        run('git', 'init', '-q')
        run('git', 'add', '-A')
        run('git', 'commit', '-q', '-m', 'Initial commit from cookiecutter-gitops-multirepo charts template')
        # Name the initial branch after the branch chart releases are tagged from
        # (default_branch), which Argo CD's dev environment tracks.
