...
link:how-it-works.adoc[How It Works] first.
...
| Add-on CI
| Apps are render-gated; add-ons are not.
| Adding a render gate for `appset-addons.yaml`.

| Helper scripts
| Parse the list-generator shape only.
| Updating them if you change the generator type.

| Bootstrap sync
| Auto-sync, no `prune`/`selfHeal`.
| Enabling `prune` + `selfHeal` for strict GitOps.

| AppProject
| Everything in `default`.
| Defining scoped `AppProject` resources.

| Cluster targeting
| By name; clusters must pre-exist.
| Registering clusters; considering label selectors.
|===
