...
This is why the same chart can run on three clusters with three different
behaviours: the chart is shared, but each cluster reads its own values file.

[source,text]
----
clusters/
  {{ cookiecutter.cluster_dev }}/
    apps/podinfo.yaml        # 1 replica, debug logs, no HPA
    addons/metrics-server.yaml
  {{ cookiecutter.cluster_stage }}/
    apps/podinfo.yaml        # 2 replicas, info logs, HPA on
    addons/metrics-server.yaml
  {{ cookiecutter.cluster_prod }}/
    apps/podinfo.yaml        # 3 replicas, larger limits, HPA on
    addons/metrics-server.yaml
----
...
* Both `ApplicationSet` resources sync automatically with `prune: true` and
  self-heal enabled, so drift is corrected and removed elements are cleaned up.
...
* `preserveResourcesOnDeletion: true` on the `ApplicationSet` means deleting the
  set does not cascade-delete the workloads it manages.
