...
* `bootstrap/appset-addons.yaml` - an `ApplicationSet` (matrix generator) for cluster add-ons, deployed at the same version to every cluster. The sample add-ons are `cert-manager`, `external-secrets`, and `metrics-server`.
